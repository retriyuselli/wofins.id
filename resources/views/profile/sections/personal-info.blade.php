...
<div>
    ...
    <div class="space-y-4">
        <div>
            ...
            <p class="text-[var(--wf-ink)] text-sm font-medium">{{ $user->name }}</p>
        </div>
        <div>
            <label class="text-sm font-medium text-[var(--wf-muted)]">Alamat Email</label>
            <p class="text-[var(--wf-ink)] text-sm font-medium">{{ $user->email }}</p>
        </div>
        <div>
            <label class="text-sm font-medium text-[var(--wf-muted)]">Nomor Telepon</label>
            <p class="text-[var(--wf-ink)] text-sm font-medium">{{ $user->phone_number ?? 'Tidak ditentukan' }}</p>
        </div>
        <div>
            <label class="text-sm font-medium text-[var(--wf-muted)]">Tanggal Lahir</label>
            <p class="text-[var(--wf-ink)] text-sm font-medium">{{ $user->date_of_birth ? $user->date_of_birth->format('d F Y') : 'Tidak ditentukan' }}</p>
        </div>
        <div>
            <label class="text-sm font-medium text-[var(--wf-muted)]">Jenis Kelamin</label>
            <p class="text-[var(--wf-ink)] text-sm font-medium">{{ $user->gender ? ucfirst($user->gender) : 'Tidak ditentukan' }}</p>
        </div>
    </div>
</div>

...
final class APIClient {
    ...
    func schedule() async throws -> ScheduleData {
        let envelope: DataEnvelope<ScheduleData> = try await request(method: "GET", path: "/me/schedule")
        return envelope.data
    }

    func leaveBalances(year: Int? = nil) async throws -> LeaveBalancesData {
        var path = "/me/leave-balances"
        if let year { path += "?year=\(year)" }
        let envelope: DataEnvelope<LeaveBalancesData> = try await request(method: "GET", path: path)
        return envelope.data
    }

    func leaveTypes() async throws -> [LeaveTypeItem] {
        let envelope: DataEnvelope<[LeaveTypeItem]> = try await request(method: "GET", path: "/me/leave-types")
        return envelope.data
    }

    func leaveRequests(status: String? = nil, year: Int? = nil) async throws -> [LeaveRequestItem] {
        var query: [String] = []
        if let status { query.append("status=\(status)") }
        if let year { query.append("year=\(year)") }
        var path = "/me/leave-requests"
        if !query.isEmpty { path += "?" + query.joined(separator: "&") }
        let envelope: PagedEnvelope<LeaveRequestItem> = try await request(method: "GET", path: path)
        return envelope.data
    }

    func leaveRequest(id: Int) async throws -> LeaveRequestItem {
        let envelope: DataEnvelope<LeaveRequestItem> = try await request(
            method: "GET",
            path: "/me/leave-requests/\(id)"
        )
        return envelope.data
    }

    func createLeaveRequest(_ payload: CreateLeavePayload) async throws -> LeaveRequestItem {
        let envelope: MessageDataEnvelope<LeaveRequestItem> = try await request(
            method: "POST",
    ...
    func absensiHariIni() async throws -> AbsensiHariIniData {
        let envelope: DataEnvelope<AbsensiHariIniData> = try await request(method: "GET", path: "/absensi/hari-ini")
        return envelope.data
    }

    func absensiLokasi() async throws -> [LokasiAbsensiItem] {
        let envelope: DataEnvelope<[LokasiAbsensiItem]> = try await request(method: "GET", path: "/absensi/lokasi")
        return envelope.data
    }

    func absensiCekLokasi(lintang: Double, bujur: Double) async throws -> CekLokasiData {
        let path = "/absensi/cek-lokasi?lintang=\(lintang)&bujur=\(bujur)"
        let envelope: DataEnvelope<CekLokasiData> = try await request(method: "GET", path: path)
        return envelope.data
    }

    func absensiRiwayat() async throws -> [AbsensiItem] {
        let envelope: PagedEnvelope<AbsensiItem> = try await request(method: "GET", path: "/absensi/riwayat")
        return envelope.data
    }

    func absensiRingkasan() async throws -> AbsensiRingkasanData {
        let envelope: DataEnvelope<AbsensiRingkasanData> = try await request(method: "GET", path: "/absensi/ringkasan")
        return envelope.data
    }
    ...
    func absensiMasuk(
        ...
        akurasi: Double?,
        fotoJPEG: Data
    ) async throws -> AbsensiItem {
        try await absensiClock(path: "/absensi/masuk", lintang: lintang, bujur: bujur, akurasi: akurasi, fotoJPEG: fotoJPEG)
    }
    ...
    func absensiPulang(
        ...
        try await absensiClock(path: "/absensi/pulang", lintang: lintang, bujur: bujur, akurasi: akurasi, fotoJPEG: fotoJPEG)
    }

    private func absensiClock(
        path: String,
        lintang: Double,
        bujur: Double,
        akurasi: Double?,
        ...
        var fields: [String: String] = [
            "lintang": String(lintang),
            "bujur": String(bujur),
            "nama_perangkat": "ios-wofins",
        ]
        if let akurasi {
            fields["akurasi_meter"] = String(akurasi)
        }
        let envelope: MessageDataEnvelope<AbsensiItem> = try await multipart(
            method: "POST",
            path: path,
            fields: fields,
            fileField: "foto",
            fileName: "absensi.jpg",
            mimeType: "image/jpeg",
            fileData: fotoJPEG
        )
        return envelope.data
